                ...
                {plans.map((plan, index) => (
                    <div
                        ...
                        className={`relative rounded-3xl p-6 sm:p-8 border-2 transition-all duration-300 flex flex-col h-full ${plan.highlighted
                            ? "border-blue-600 bg-white shadow-2xl lg:scale-105 z-10"
                            : "border-gray-50 bg-[#F9FAFB] hover:border-blue-100"
                            }`}
                    >
                        {plan.badge && (
                            <div className="absolute top-0 right-10 -translate-y-1/2 bg-blue-600 text-white text-sm font-bold px-4 py-1.5 rounded-full tracking-wider">
                                {plan.badge}
                            </div>
                        )}

                        <div className="mb-8">
                            <h3 className={`text-xl font-bold mb-4 ${plan.highlighted ? "text-blue-600" : "text-gray-900"}`}>
                                {plan.name}
                            </h3>
                        ...
                        <div className="flex-grow space-y-5 mb-10">
                            {plan.features.map((feature, fIndex) => (
                                <div key={fIndex} className="flex items-center gap-3">
                                    {feature.included ? (
                                        <CheckCircle2 size={18} className={plan.highlighted ? "text-blue-500" : "text-green-500"} />
                                    ) : (
